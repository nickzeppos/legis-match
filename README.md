...
[^5]: Segmenting a problem like this is a very common approach in information retrieval, and both papers cited above use it. In Wilkerson, 2015, they only compute alignment scores between sections that share 5 10-grams (p. 41). In Linder, 2020, they use an off-the-shelf solution, ElasticSearch's "more like this" query option (pp. 43-44). The intuition here, for us, is simple: sections that are semantically similar are the most likely to have high alignment scores.